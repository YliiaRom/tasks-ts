...
let btn = document.getElementById("btnTask6") as HTMLButtonElement;
let btn2 = document.getElementById("btnTask6-b") as HTMLButtonElement;

let message = document.getElementById("solution6") as HTMLElement;
...
btn?.addEventListener("click", () => {
  ...
  if (amount && currency) {
    switch (currency.toUpperCase()) {
      case "USD":
        result = `Ви ввели : ${amount}USD, це ${(amount * USD_TO_UAH).toFixed(
          2
        )}UAH, і  це ${(amount / EUR_TO_USD).toFixed(2)}EUR,`;
        break;
      case "EUR":
        result = `Ви ввели : ${amount}EUR, це ${(amount * EUR_TO_UAH).toFixed(
          2
        )}UAH, і  це ${(amount * EUR_TO_USD).toFixed(2)}USD,`;
        break;
      case "UAH":
        result = `Ви ввели : ${amount}UAH, це ${(amount / USD_TO_UAH).toFixed(
          2
        )}USD, і  це ${(amount / EUR_TO_UAH).toFixed(2)}EUR,`;
        break;
      default:
...
btn2?.addEventListener("click", () => {
  ...
  //--value
  const currencyValue = currency.toUpperCase() as Currency;
  ...
  switch (currencyValue) {
    case "USD":
      result = `Ви ввели: ${sum} USD → ${(sum * USD_TO_UAH).toFixed(2)} UAH, ${(
        sum / EUR_TO_USD
      ).toFixed(2)} EUR`;
      break;

    case "EUR":
      result = `Ви ввели: ${sum} EUR → ${(sum * EUR_TO_UAH).toFixed(2)} UAH, ${(
        sum * EUR_TO_USD
      ).toFixed(2)} USD`;
      break;

    case "UAH":
      result = `Ви ввели: ${sum} UAH → ${(sum / USD_TO_UAH).toFixed(2)} USD, ${(
        sum / EUR_TO_UAH
      ).toFixed(2)} EUR`;
      break;
